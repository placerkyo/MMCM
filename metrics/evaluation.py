...
def apd(target, pred, *args, t0=0, t=-1):
    pred = time_slice(pred, t0, t, 2).cpu()
    # (batch_size, num_samples, seq_length, num_parts, num_joints, features) to (num_samples, all_others)   #torch.Size([32, 50, 100, 1, 16, 3])
    ...
    # bs_indices = np.repeat(np.arange(batch_size), pdist_shape)
    # values_mask = np.tile(iu, (batch_size, 1))
    # final_mask = (bs_indices, values_mask[0], values_mask[1])
...
# def apd_humanmac(target, pred, *args):   # target.shape: [1, 100, 1, 16, 3], pred.shpae: [1, 50, 100, 1, 16, 3]
#     if pred.shape[1] == 1:
#         return torch.tensor([0.0])
#     dist_diverse = torch.pdist(pred.reshape(pred.shape[1], -1))
#     diversity = dist_diverse.mean()
#     return torch.tensor([diversity])

def apd_humanmac(target, pred, *args):
    div_list = []
...
def get_multimodal_gt(data_loader, multimodal_threshold, num_frames=1, device='cpu', split='test'):
    ...
    for batch_data in data_loader:
        ...
        all_last_obs.append(data[:, -num_frames:].reshape(data.shape[0], -1))
