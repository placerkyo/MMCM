...
from autoencoder.models.basics import rc, rc_recurrent, reparametrize_logstd, RNN, MLP
...
class BEncoder(nn.Module):
    ...
    def forward(self, x, sample=False, deterministic=False):
...
class ResidualBehaviorNetLike(BaseModel):
    def __init__(self, n_features, n_landmarks, obs_length, pred_length, 
                dim_hidden_b=128, dim_decoder_state=128, decoder_arch='lstm', linear_nin_decoder=False,
                residual=False, recurrent=False):
        ...
        self.n_kps = n_features * n_landmarks
    ...
    def forward(self, x, y=None, deterministic=False):
        ...
        b, mu, logstd, pre = self.infer_b(x, deterministic=deterministic)
        xs, _ = self.generate_seq(b)
    ...
    def infer_b(self, s, deterministic=False):      # p_theta
        """
        ...
        outs = self.b_enc(s, deterministic=deterministic)
        return outs
    ...
    def generate_seq(self, b):
        ...
        xs = torch.zeros([batch_size, self.obs_length, self.n_kps], device=b.device)

        self.decoder.init_hidden(batch_size, device=b.device)

        for i in range(self.obs_length): # we now decode the other part of the sequence (not context)
            x = self.decoder.forward_noresidual(b)
            xs[:, i] = x
