...
class AnimationRenderer:
    ...
    def __init__(self, skeleton, poses_generator, algos, t_hist, t_pred, baselines=['gt', 'context'], 
        ...
        self.poses_dict = dict(filter(lambda x: x[0] in baselines or self.algo == x[0].split('_')[0], self.all_poses.items()))   # 表示するGTとContextとサンプル
    ...
    def run_animation(self, show=True):
        ...
        self.lines = []
        if self.fig_3d:        # True
            self.radius, x_r, y_r, z_r = self.skeleton.get_3d_radius()        # 5, [-2.5, 2.5], [-2.5, 2.5], [-2.5, 2.5] 3Dプロットの半径と軸範囲?
    ...
    def store_all(self, type="gif", idx=-1):
        ...
        self.run_animation(show=False)             # 3Dプロットの準備と設定
        self._reload_poses()                       # ポーズデータの再ロードと3Dプロットの設定の更新
    ...
    def _update_video(self, i):
        ...
        if i < self.t_hist:     # 線の太さ
            linewidth = 1
        else:
            linewidth = 2

        parents = self.skeleton.parents()      # 関節の親子関係
            ...
            for n, ax in enumerate(self.axes):
                ...
                for k in range(self.poses[n].shape[1]):
                    ...
                    for j, j_parent in enumerate(parents):
                        ...
                        zorder = 2 + mid        # オブジェクトが重なっていた時この値が大きい方が前面に描画される
        ...
        if not self.initialized:        # 初期化がまだの時？
